...
- Canonical components/API contracts touched: `forum_web.web` WSGI contract; `cgi-bin/create_thread.py`; `cgi-bin/create_reply.py`; new PHP-primary operator install guide.

## Stage 2
- Goal: add the minimal PHP-facing read adapter and Apache-style routing files for the public site surface.
- Dependencies: Stage 1; existing read-only route contract in `forum_web.web`.
...
- Canonical components/API contracts touched: public route contract from `forum_web.web`; PHP front-controller entrypoint; `.htaccess` rewrite behavior.
