...
class BoardIndexPageTests(ForumRepoTestCase):
    ...
    def test_board_index_preserves_key_destination_links(self) -> None:
        ...
        self.assertIn('<nav class="site-header-nav" data-primary-nav aria-label="Primary">', body)
        self.assertIn('<a data-primary-nav-link href="/" aria-current="page">Home</a>', body)
        self.assertIn('/compose/thread', body)
        self.assertIn('/instance/', body)
        self.assertIn('/activity/', body)
        self.assertIn('data-primary-nav-link', body)
        ...
        self.assertIn('/assets/primary_nav.js', body)
